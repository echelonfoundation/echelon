...
A user can query the Echelon `x/vesting` module using the CLI, gRPC, or REST.

## CLI

Find below a list of `echelond` commands added with the `x/vesting` module. You can obtain the full list by using the `echelond -h` command.
...
```go
echelond query vesting balances [address] [flags]
```
...
```go
echelond tx vesting create-clawback-vesting-account [to_address] [flags]
```
...
```go
echelond tx vesting clawback [address] [flags]
```
...
| ------ | -------------------------------------- | -------------------------------------- |
| `gRPC` | `echelon.vesting.v1.Query/Balances`      | Gets locked, unvested and vested coins |
| `GET`  | `/echelon/vesting/v1/balances/{address}` | Gets locked, unvested and vested coins |

### Transactions

| Verb   | Method                                                 | Description                      |
| ------ | ------------------------------------------------------ | -------------------------------- |
| `gRPC` | `echelon.vesting.v1.Msg/CreateClawbackVestingAccount`    | Creates clawback vesting account |
| `gRPC` | `/echelon.vesting.v1.Msg/Clawback`                       | Performs clawback                |
| `GET`  | `/echelon/vesting/v1/tx/create_clawback_vesting_account` | Creates clawback vesting account |
| `GET`  | `/echelon/vesting/v1/tx/clawback`                        | Performs clawback                |
