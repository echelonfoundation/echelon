...
func (k Keeper) MergeClaimsRecords(
	...
	for _, action := range actions {
		...
		//  - the sender is not an echelon address and can't claim vote, delegation or evm actions
		...
		if senderClaimsRecord.HasClaimedAction(action) {
			return types.ClaimsRecord{}, sdkerrors.Wrapf(sdkerrors.ErrNotSupported, "non-echelon sender must not have claimed action: %v", action)
		}
