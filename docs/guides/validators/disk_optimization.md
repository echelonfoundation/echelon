...
speed and transaction amount. For Echelon, we are talking about close to
...
I believe this was disabled by default on Echelon, but listing it in any
...
will risk database corruption if the `echelond` is killed for any reason.
...
Below is the disk usage after two weeks of Echelon Arsia Mons testnet. The default
