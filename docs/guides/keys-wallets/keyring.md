...
```bash
echelond keys add mykey

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(echelond keys show mykey -a)
```
...
::: warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Echelon due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | echelond keys add me
yes $KEYPASSWD | echelond keys show me
# start echelond with keyring-backend flag
echelond --keyring-backend=file start
```
