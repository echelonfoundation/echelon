...
Learn how Echelon handles pending state queries. {synopsis}
...
Echelon is designed quite differently on this front as there is no concept of a "pending state".
Echelon uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Echelon cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool.html#transaction-ordering).

## Pending State Queries

Echelon will make queries which will account for any unconfirmed transactions present in a node's
...
Learn how to deploy a Solidity smart contract on Echelon using [Truffle](./../guides/truffle.md) {hide}
